# Установка модов — TLauncher / официальный Minecraft Launcher

Два разных сервера — два разных набора шагов, не перепутайте:

- **Основной сервер** (`stulbia.qqwrd.com:443` / `38.180.167.101:443`) — Fabric, MC 26.2, см. раздел ниже.
- **Тестовый сервер с Create** (`stulbia.qqwrd.com:25566` / `38.180.167.101:25566`) — NeoForge, MC 1.21.1, отдельный раздел в самом низу.

---

# Основной сервер (Fabric, MC 26.2)

## Шаг 1. Поставить Fabric

Скачайте официальный установщик Fabric: https://fabricmc.net/use/installer/

Запустите его, вкладка **Client**:
- Minecraft version: **26.2**
- Loader version: **0.19.3**
- Install location: оставьте по умолчанию (это создаст обычный `.minecraft` профиль)

Нажмите Install. Появится профиль `fabric-loader-0.19.3-26.2`.

- **Официальный лаунчер** — профиль появится в списке автоматически.
- **TLauncher** — откройте вкладку профилей, найдите/добавьте профиль `fabric-loader-0.19.3-26.2` (TLauncher обычно подхватывает его сам из `.minecraft/versions`, либо создайте новый профиль и укажите эту версию вручную).

## Шаг 2. Найти папку игры (game directory)

Это папка, где лежат `mods`, `config`, `saves` и т.д.

- **Официальный лаунчер**: обычно `%appdata%\.minecraft` (Windows) / `~/.minecraft` (Linux) / `~/Library/Application Support/minecraft` (Mac).
- **TLauncher**: откройте настройки профиля → там указан путь (может отличаться, например `%appdata%\.tlauncher\minecraft`).

## Шаг 3. Скачать packwiz-installer-bootstrap

https://github.com/packwiz/packwiz-installer-bootstrap/releases — скачайте `packwiz-installer-bootstrap.jar`.

Положите этот jar-файл **прямо в папку игры** (ту, что нашли в Шаге 2, где лежит папка `mods`).

## Шаг 4. Запустить установку модов

Откройте терминал/командную строку **в этой самой папке** (на Windows: зайти в папку → в адресной строке проводника набрать `cmd` и нажать Enter) и выполните:

```
java -jar packwiz-installer-bootstrap.jar https://raw.githubusercontent.com/vox4vulpes/v4f-Minecraft-pack/main/pack.toml
```

Моды скачаются в папку `mods` автоматически.

## Обновления

Модпак может обновляться. Автоматически это не подхватится (в отличие от Prism Launcher) — **перед запуском игры** нужно заново выполнить команду из Шага 4, чтобы подтянуть актуальные версии модов.

## Запуск

Выберите профиль `fabric-loader-0.19.3-26.2` в лаунчере и запускайте игру как обычно.

---

# Тестовый сервер с Create (NeoForge, MC 1.21.1)

Отдельный сервер для тестов, **не связан** с packwiz-модпаком выше. Адрес: `stulbia.qqwrd.com:25566` или `38.180.167.101:25566` (оба ведут на один и тот же сервер).

## Быстрый способ — автоустановщик (Windows)

Скрипт делает почти всё сам: ставит NeoForge, создаёт отдельную папку игры, скачивает Create через packwiz. Есть два варианта — под официальный лаунчер и под TLauncher.

**Общее для обоих:**
- Требуется Java 21+ в PATH (проверить: `java -version` в командной строке). Если нет — поставьте с https://adoptium.net/.
- Не запускайте двойным кликом — Windows блокирует скачанные скрипты. Откройте PowerShell в папке со скриптом (Shift + правый клик в папке → "Открыть окно PowerShell здесь") и выполните команду из соответствующего пункта ниже.
- Ничего в существующих сохранениях/модах/профилях не удаляется и не перезаписывается — только добавляется новое.

### Официальный лаунчер

Скрипт также сам добавляет профиль в лаунчер.

1. Скачайте: https://raw.githubusercontent.com/vox4vulpes/v4f-Minecraft-pack/main/test-server/install-create-test.ps1
2. Выполните:
   ```
   powershell -ExecutionPolicy Bypass -File install-create-test.ps1
   ```
3. Дождитесь "Готово!", перезапустите лаунчер, выберите профиль **v4f Create Test**.

Скрипт делает резервную копию `launcher_profiles.json` перед правкой (файл `.bak-...` рядом).

### TLauncher

У TLauncher своя, недокументированная система профилей — автоматически дописывать её вслепую рискованно. Поэтому этот вариант скрипта делает только безопасную файловую часть (NeoForge + отдельная папка + моды), а профиль вы добавляете сами — 2 клика в самом TLauncher.

1. Скачайте: https://raw.githubusercontent.com/vox4vulpes/v4f-Minecraft-pack/main/test-server/install-create-test-tlauncher.ps1
2. Выполните:
   ```
   powershell -ExecutionPolicy Bypass -File install-create-test-tlauncher.ps1
   ```
   (по умолчанию скрипт ставит NeoForge в `%appdata%\.minecraft` — если у TLauncher настроена другая папка, посмотрите путь в его настройках и запустите с `-McDir "путь"`)
3. В конце скрипт покажет путь к отдельной папке игры (что-то вроде `%appdata%\.minecraft-v4f-create-test`) — откройте TLauncher, добавьте/выберите профиль версии `neoforge-21.1.249` (появится в списке версий) и укажите этот путь как «Директорию игры» в настройках профиля.
4. Запускайте, подключайтесь к `stulbia.qqwrd.com:25566`.

Если что-то пошло не так с любым из скриптов — используйте ручной способ ниже.

## Ручной способ

### Шаг 1. Поставить NeoForge

Скачайте установщик: https://neoforged.net/

- Minecraft version: **1.21.1**
- NeoForge version: **21.1.249**
- Выберите "Install client", оставьте путь по умолчанию.

Появится профиль вида `neoforge-21.1.249`.

- **Официальный лаунчер** — профиль появится сам во вкладке **Installations**. Если сразу не видно — **перезапустите лаунчер** (частая история, установщик пишет профиль, пока лаунчер уже открыт).
- **TLauncher** — найдите/добавьте этот профиль так же, как с Fabric выше.

Если профиля всё равно нет в списке версий — установщик мог поставить файлы не в ту `.minecraft`. Проверьте, есть ли папка `.minecraft\versions\neoforge-21.1.249` (Windows: `%appdata%\.minecraft\versions\`); если нет — переустановите NeoForge, явно указав установщику путь к вашей настоящей `.minecraft` (поле «Installation directory» / «Client install location»). Для TLauncher — то же самое, но путь берётся из настроек самого TLauncher (свой отдельный `.minecraft`, не совпадает с официальным).

### Шаг 2. Отдельная папка игры (обязательно!)

⚠️ Одна и та же папка `mods` не может одновременно содержать и Fabric-моды основного сервера, и Create — они несовместимы друг с другом (разные загрузчики). Профиль `neoforge-21.1.249` должен использовать **отдельную** папку игры, а не общую `.minecraft`.

**Официальный лаунчер:**
1. Вкладка **Installations** → найдите `neoforge-21.1.249` → **⋮** (три точки) → **Edit**.
2. Разверните **More Options**.
3. В поле **Game Directory** укажите новую, отдельную папку (например `D:\minecraft-create`).
4. Сохраните.

**TLauncher:** в настройках профиля есть аналогичное поле «Директория игры» / «Game directory» — укажите там отдельную папку так же.

### Шаг 3. Поставить мод Create (автоматически через packwiz)

Скачайте `packwiz-installer-bootstrap.jar` (см. Шаг 3 в разделе основного сервера выше, если ещё не скачивали).

Положите `packwiz-installer-bootstrap.jar` в ту самую отдельную папку игры из Шага 2 и выполните:

```
java -jar packwiz-installer-bootstrap.jar https://raw.githubusercontent.com/vox4vulpes/v4f-Minecraft-pack/main/test-server/pack.toml
```

Create скачается в `mods` автоматически. При обновлении мода — просто повторите эту команду перед запуском.

### Шаг 4. Запуск

Выберите профиль `neoforge-21.1.249`, подключайтесь к `stulbia.qqwrd.com:25566`.

Сервер в offline-режиме — доступен под любым ником, авторизация Mojang не требуется.
